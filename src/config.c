...
config_params_s init_config() {
  ...
  c.gamepad_select = SDL_CONTROLLER_BUTTON_Y;
  c.gamepad_start = SDL_CONTROLLER_BUTTON_B;
  c.gamepad_opt = SDL_CONTROLLER_BUTTON_X;
...
void read_gamepad_config(ini_t *ini, config_params_s *conf) {
  ...
  if (gamepad_analog_invert && strcmpci(gamepad_analog_invert, "true") == 0)
